//audio feather, will download files from YouTube and keep track of the queue.
module.exports = function audioHandler(bot, info)
{
        ...
        bot.getAudioContext(info.config.voice_channel, function(err, stream)
        {
          ...
          stream.on('done', function()
          {
            ...
            else
            {
              ...
              audio.play(audio.playlist[0].file, audio.playlist[0].image, audio.playlist[0].song_name);
            }
  ...
  audio.play = function(song, im, sn)
  {
    ...
    console.log('Song: ' + song + ' Image Link: ' + im);
  };
  ...
  audio.downloader.on('message', function(m)
  {
    if(m.status == 0)
    {
      info.audio.playlist.push({'song_name': m.title, 'file': m.file, 'image':m.image});
